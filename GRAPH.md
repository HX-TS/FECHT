  ...
  <script>
    ...
    */
    const STATIONS = {
      gps: {
        name: "GPS",
        spreadsheetId: "1OucgLhFiTqkeWiXJ0aI7j_tMsu9lVnqTxSK8y2AMCn4",
        gid: "0",
        canvasId: "gpsChart",
    ...
    const HOLD_START = new Date("2026-08-21T21:30:00+08:00");
    const HOLD_END = new Date("2026-08-22T21:30:00+08:00");
    ...
    */
    ...
    class DualAxisTimeChart {
      ...
      drawXAxis(plot, firstTime, lastTime, xForTime) {
        const ctx = this.ctx;
        const stepMs = X_AXIS_TICK_MINUTES * 60 * 1000;
    ...
    function updateHoldingTimer() {
      const now = new Date();
      const totalMs = Math.max(0, HOLD_END - HOLD_START);
      const elapsedMs = Math.min(totalMs, Math.max(0, now - HOLD_START));
      const remainingMs = Math.max(0, HOLD_END - now);
    ...
    function tableToChartRows(table) {
      ...
      const columns = {
        ...
        time: findColumn(headers, COLUMN_ALIASES.time),
        dwt: findColumn(headers, COLUMN_ALIASES.dwt),
        digital: findColumn(headers, COLUMN_ALIASES.digital),
        recorder: findColumn(headers, COLUMN_ALIASES.recorder),
        ambient: findColumn(headers, COLUMN_ALIASES.ambient),
        water: findColumn(headers, COLUMN_ALIASES.water)
      };
    ...
    function findHeaderRow(table) {
      ...
      for (let i = 0; i < scanCount; i++) {
        ...
        if (findColumn(headers, COLUMN_ALIASES.time) >= 0 || findColumn(headers, COLUMN_ALIASES.date) >= 0) score += 2;
        if (findColumn(headers, COLUMN_ALIASES.dwt) >= 0) score += 2;
        if (findColumn(headers, COLUMN_ALIASES.digital) >= 0) score += 2;
        if (findColumn(headers, COLUMN_ALIASES.recorder) >= 0) score += 2;
        if (findColumn(headers, COLUMN_ALIASES.ambient) >= 0) score += 2;
        if (findColumn(headers, COLUMN_ALIASES.water) >= 0) score += 2;
